      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {data && data.length === 0 ? (
          <div className="flex flex-col items-center text-center py-24">
            <div className="w-18 h-18 rounded-full bg-motive-soft flex items-center justify-center mb-6">
              <SearchXIcon
                size={32}
                className="text-motive"
                strokeWidth={1.8}
              />
            </div>
            ...
            <p className="text-[13px] text-ink-400 leading-relaxed mb-7">
              검색어나 필터를 조정하면
              <br />더 많은 스터디를 찾을 수 있어요
            </p>
            {onResetFilter && (
              <button
                type="button"
                onClick={onResetFilter}
                className="text-[13px] font-bold text-motive bg-motive-soft hover:bg-motive-light transition-colors duration-150 rounded-[10px] px-5 py-2.5"
              >
                필터 초기화
              </button>
            )}
