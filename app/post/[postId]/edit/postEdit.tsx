...
import MotiveDatePicker from "@/components/MotiveDatePicker";
...
function FieldLabel({
  ...
  return (
    <label className="text-[14px] font-semibold text-ink-800">
      {children}
      {required && <span className="text-motive ml-0.5">*</span>}
    </label>
  );
}
  ...
  return (
    <div className="relative">
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full h-11.5 px-4 border border-ink-200 rounded-[10px] text-[14px] text-ink-900 placeholder:text-ink-300 bg-white outline-none focus:border-motive transition-colors"
      />
      {rightIcon && (
...
export default function PostEdit() {
   ...
   * TODO: 게시글 수정 API 연동
   */
  const onsubmit = (data: PostModifyRequest) => {
    modifyPost(
      { postId, body: { ...data, recruitPositions } },
      {
        onSuccess: ({ data }) => {
          ...
          <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2 mt-1">
                  {recruitPositions.map((pos) => {
                    const nm =
                      ...
                      pos.recruitPositTypeCd;
                    return (
                      <div
                        key={pos.recruitPositTypeCd}
                        className="flex items-center justify-between px-4 py-2.5 bg-motive-soft border border-motive/20 rounded-[10px]"
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-[13px] font-bold text-motive">
                            {nm}
                          </span>
                  ...
                  <p className="text-[12px] text-ink-400 text-right">
                    전체 모집인원:{" "}
                    <span className="font-bold text-motive">
                      {totalRecruitCnt}명
                    </span>
                  </p>
                </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full min-h-40 px-4 py-3 border border-ink-200 rounded-[10px] text-[14px] text-ink-900 placeholder:text-ink-300 bg-white outline-none focus:border-motive transition-colors resize-y leading-relaxed"
                  {...register("content")}
                />
              </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                className="w-full min-h-25 px-4 py-3 border border-ink-200 rounded-[10px] text-[14px] text-ink-900 placeholder:text-ink-300 bg-white outline-none focus:border-motive transition-colors resize-y leading-relaxed"
