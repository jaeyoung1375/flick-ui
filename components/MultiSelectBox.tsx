      ...
      <Popover.Root open={open} onOpenChange={setOpen}>
        <Popover.Trigger
          ...
          className={cn(
            ...
            "focus:outline-none focus:ring-2 focus:ring-motive/20 focus:border-motive",
          ...
          <div className="flex flex-wrap gap-1.5 flex-1">
            {selectedLabels.length === 0 ? (
              ...
              selectedLabels.map((opt) => (
                <span
                  key={opt.value}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-[6px] bg-motive-soft text-motive text-[12px] font-semibold"
                >
                  {opt.label}
                  <span
                    role="button"
                    ...
                    className="hover:text-motive/60 transition-colors cursor-pointer"
                  >
                    <X className="w-3 h-3" />
                  </span>
                </span>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected
                      ? "bg-motive-soft text-motive font-semibold"
                      : "text-ink-900 hover:bg-motive-soft hover:text-motive",
                  )}
                >
                  <span>{opt.label}</span>
                  {selected && <Check className="w-3.5 h-3.5 text-motive" />}
                </button>
              );
            })}
          </Popover.Content>
