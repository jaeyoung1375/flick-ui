# 테스트 전략 (flick-ui Test Strategy)

## 1. 현재 상태 (2026-09-16 실측)
...
* `.github/workflows/deploy.yml`은 `npm ci` → `npm run build` → 배포로 바로 이어진다 — 테스트 단계 자체가 파이프라인에 없다(백엔드처럼 "스킵"하는 게 아니라 애초에 구성되어 있지 않음).
...
* E2E 성격의 검증(소셜 로그인 리다이렉트, 향후 OTT 재생 흐름 등)이 필요하면 실제 백엔드(`flick-server`)와의 통합이 전제이므로, 단위 테스트만으로는 흐름 전체를 검증할 수 없다는 점을 감안한다.
