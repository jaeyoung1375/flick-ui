# 배포 런북 상세 (jobmoa-ui Deploy Runbook)

실측: `.github/workflows/deploy.yml` (2026-08-10 기준)
...
사전 조건: 백엔드(`motive-server`)가 `NEXT_PUBLIC_API_BASE_URL`(기본 `http://localhost:9090`)에서 기동 중이어야 API 호출이 성공한다.
...
* **러너:** `ubuntu-latest` (GitHub 호스팅, motive-server의 self-hosted와 다름)
* **단계:**
  1. `actions/checkout@v3`
  2. `actions/setup-node@v3` (Node 20, npm 캐시)
  3. `npm ci`
  4. `npm run build` — `env: NEXT_PUBLIC_API_URL: http://146.56.116.158:9090` 설정 ⚠️ **이 변수명은 코드에서 쓰이지 않는다**(`util/AxiosUtil.ts`는 `NEXT_PUBLIC_API_BASE_URL`을 읽음) — 실제 값은 `.env.production`에서 온다. [`12_known_issues`](../12_known_issues/jobmoa-ui-known-issues.md) 7번 참고
...
* 대상 서버는 Node.js(nvm 경유) + **pm2**로 프로세스를 관리한다(motive-server가 systemd를 쓰는 것과 다른 방식).
...
- [ ] `.env.production`의 OAuth URL 경로 패턴(`/oauth/{provider}`)이 백엔드와 실제로 맞는지 확인([`12_known_issues`](../12_known_issues/jobmoa-ui-known-issues.md) 6번)
