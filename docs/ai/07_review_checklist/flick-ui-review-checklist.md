...
## 인증
- [ ] 로그인 필요 화면에서 로딩/비로그인 상태를 실제로 구분해 렌더링했는가 (children을 무조건 렌더링하는 자리표시자 가드를 새로 만들지 않았는가 — `AdminAuthGuard` 사례 참고, [`12_known_issues`](../12_known_issues/flick-ui-known-issues.md))
- [ ] 새 관리자 화면을 추가했다면, 현재 프론트·백엔드 양쪽에 role 기반 접근 제어가 없다는 한계를 인지하고 있는가
- [ ] 백엔드에서 이미 제거된 `onboardingCompleted` 같은 필드를 되살려 쓰지 않았는가

## API 연동
- [ ] 호출하는 엔드포인트가 `flick-server`의 `docs/ai/09_api_contract`에 실제로 존재하는지 확인했는가(존재하지 않으면 임의로 있다고 가정하지 않았는가 — `features/admin` 사례 참고)
...
- [ ] 새 의존성을 추가했다면 [`02_tech_stack`](../02_tech_stack/flick-ui-tech-stack.md)에도 반영했는가

## 문서 동기화
- [ ] 소비하는 API가 바뀌었다면 [`09_api_contract`](../09_api_contract/flick-ui-api-contract.md)를 갱신했는가
- [ ] 클라이언트 타입/전역 상태 구조가 바뀌었다면 [`10_data_model`](../10_data_model/flick-ui-data-model.md)을 갱신했는가
- [ ] 변경 사항을 [`11_changelog`](../11_changelog/flick-ui-changelog.md)에 남겼는가
- [ ] 새로 발견한 함정이 있다면 [`12_known_issues`](../12_known_issues/flick-ui-known-issues.md)에 남겼는가
