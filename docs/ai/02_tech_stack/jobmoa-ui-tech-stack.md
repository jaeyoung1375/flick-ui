# 기술 스택 상세 (jobmoa-ui Tech Stack)

실측 근거: `package.json`, `tsconfig.json`, `eslint.config.mjs`, `next.config.js`, `postcss.config.mjs` (2026-08-10 기준)
...
* **Zustand 5.0.12** — 로그인이 아닌 전역 상태(`store/`: `alertStore`, `confirmStore`, `onboardingStore`, `recordDraftStore`)
...
* **Tailwind CSS 4.2.1** + `@tailwindcss/postcss` — `postcss.config.mjs`에 플러그인 등록, `styles/globals.css`의 `@theme`에서 `--color-motive`(#ff7a00 계열, 오렌지) · `--color-ink-*` 토큰 정의
* **@tailwindcss/typography** — 에디터(Tiptap) 콘텐츠 렌더링용
* **clsx 2.1.1** + **tailwind-merge 3.5.0** — `util/cn.ts`의 클래스 병합 유틸

> ⚠️ `CLAUDE.md`에 적힌 "포인트 블루 `#2F80FF`" 디자인 컨셉과 `styles/globals.css`의 실제 테마 토큰(`--color-motive: #ff7a00`, 오렌지)이 어긋난다. 구 컴포넌트(`components/Button.tsx` 등)는 `#2F80FF`를 하드코딩하고, 신 컴포넌트(`components/ui/Button.tsx`)는 `bg-motive` 토큰(오렌지)을 쓴다 — [`12_known_issues`](../12_known_issues/jobmoa-ui-known-issues.md) 참고.
...
* **Tiptap 3.20** (`@tiptap/react`, `starter-kit`, `extension-image`, `extension-placeholder`) — `components/Editor.tsx`, 게시글/설명 작성용
...
* **lucide-react** — 아이콘
* **date-fns**, **react-datepicker** — 날짜 처리/선택 UI(`components/MotiveDatePicker.tsx`, `BirthDatePicker.tsx`)
...
새 캐러셀·jQuery 필요 작업을 시작하기 전에 이미 설치된 이 의존성들을 재사용할 수 있다고 가정하지 말 것 — 실제 사용 여부를 다시 확인한다. 상세는 [`12_known_issues`](../12_known_issues/jobmoa-ui-known-issues.md) 참고.

## 8. 빌드·배포

* Next.js 표준 빌드(`npm run build`), 배포는 GitHub Actions(`workflow_dispatch`) → `appleboy/scp-action`으로 서버 전송 → `pm2 start npm --name next-app -- start`. 상세는 [`13_deploy_runbook`](../13_deploy_runbook/jobmoa-ui-deploy-runbook.md).

## 9. 설정 파일 구조

| 파일 | 역할 |
|------|------|
| `.env.development` | 로컬 개발 — API 베이스 `localhost:9090`, OAuth 진입점 URL 3종(`/oauth2/authorization/{provider}`) |
| `.env.production` | 운영 — API 베이스 `146.56.116.158:9090`, OAuth URL이 `/oauth/{provider}` 형태로 개발 환경과 경로 패턴이 다름([`12_known_issues`](../12_known_issues/jobmoa-ui-known-issues.md) 참고) |
